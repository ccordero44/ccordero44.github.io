function LiveSpellInstance($setup) {
    ...
    this.docUpdate = function(docs) {
        ...
        for (var editedfields = new Array, i = 0; i < fieldIds.length; i++) {
            ...
            if (JavaScriptSpellCheck && oField.MessageHolder && JavaScriptSpellCheck.LiveValidateMech(oField),
            docs[i] !== oField[t]) {
                if (editedfields[editedfields.length] = id,
                "iframe" === t) {
                ...
                "value" === t && livespell.insitu.proxyDOM(id) && (livespell.insitu.updateProxy(id),
    ...
    this.docFocus = function(index) {
        for (var fieldIds = this.arrCleanFields(), i = 0; i < fieldIds.length; i++) {
            ...
            i == index ? (oField.className = "" + oField.className + " livespell_focus_glow",
            oField.scrollIntoView(!1)) : oField.className = oField.className.replace(/livespell_focus_glow/g, "")
        }
    }
    ,
    ...
    this.setFormVals = function(CaseSensitive, IgnoreAllCaps, IgnoreNumeric, CheckGrammar) {
        ...
        this.isUniPacked && $Spelling && ($Spelling.CaseSensitive = CaseSensitive,
    ...
    this.CheckInWindow = function() {
        var Dwidth = 460
          , Dheight = 290;
        livespell.test.chrome() && (Dwidth += 5,
        Dheight += 5),
        this.UserSpellingInitiated = !0,
        ...
        if ("jquery.ui" == wm.substr(0, 9) && (jQuery,
        !0) && ($.ui,
        !0)) {
            var dialogClass = null;
            this.WindowMode.length > 10 && (dialogClass = this.WindowMode.substr(10)),
            $("#livespell_jquery_ui_modal_frame").length || $("body").append("<div id='livespell_jquery_ui_modal' style='display:overflow:hidden'><iframe width='" + Dwidth + "' height='" + Dheight + "' scrolling='no' marginwidth='0' marginheight='0' frameborder=0 id='livespell_jquery_ui_modal_frame' src='about:blank'></iframe></div>");
            var f = this
              , u = this.url()
            ...
            return dialogClass && (settings.dialogClass = dialogClass),
            void $("#livespell_jquery_ui_modal").dialog(settings)
        }
        if ("fancybox" == wm && (jQuery,
        !0)) {
            var uri = this.url()
              , ttl = livespell.lang.fetch(this.id(), "WIN_TITLE");
            $.fancybox({
    ...
    this.m_AYTAjaxInervalHandler = function() {
        ...
        if (fieldIds.length) {
            for (var i = 0; i < fieldIds.length; i++) {
                ...
                if (oChild) {
                    var found = !1;
                    oChild.isCurrentAjaxImplementation !== !0 && (oChild.isCurrentAjaxImplementation = !0,
                    found = !0)
                }
            }
    ...
    this.getFieldWordListMech = function() {
        ...
        for (var i = 0; i < tokens.length; i++)
            if (livespell.test.isword(tokens[i]) === !0) {
                ...
                !mwm == !0 && (wasfine = !1),
                "undefined" == typeof mwm && (token === token.toUpperCase() || /[0-9]/gi.test(token) || this.IgnoreNumeric && livespell.test.num(token) || (wordlist = livespell.array.safepush(wordlist, token)))
            }
        return result = {},
    ...
    this.AjaxSpellCheck = function(input, makeSuggestions) {
        ...
        return allFound ? void this.onSpellCheckFromCache(input, makeSuggestions) : void (input.join ? (input = input.join(livespell.str.chr(1)),
        livespell.ajax.send("APISPELLARRAY", input, this.Language, makeSuggestions ? "" : "NOSUGGEST", this.id())) : livespell.ajax.send("APISPELL", input, this.Language, makeSuggestions ? "" : "NOSUGGEST", this.id()))
    }
    ,
    this.SpellCheckSuggest = function(input) {
        ...
        return livespell.ajax.needsInstantSuggestion = !0,
        input.join ? (input = input.join(livespell.str.chr(1)),
        livespell.ajax.send_sync("APISPELLARRAY", input, this.Language, "", this.id())) : livespell.ajax.send_sync("APISPELL", input, this.Language, "", this.id())
    }
    ,
    this.BinSpellCheck = function(input) {
        ...
        return livespell.ajax.needsInstantSuggestion = !1,
        input.join ? (input = input.join(livespell.str.chr(1)),
        livespell.ajax.send_sync("APISPELLARRAY", input, this.Language, "NOSUGGEST", this.id())) : livespell.ajax.send_sync("APISPELL", input, this.Language, "NOSUGGEST", this.id())
    }
    ,
        ...
        for (var out = new Array, j = 0; j < F.length; j++) {
            ...
            if ("ALL" === oid.toUpperCase() || "ENABLED" === oid.toUpperCase())
                ...
                i = 0; i < AF.length; i++) {
                    ...
                    "textarea" == nn && livespell.insitu.filterTextAreas(AF[i]) && ("ALL" !== oid.toUpperCase() && (AF[i].disabled || AF[i].readOnly) || (out = livespell.array.safepush(out, AF[i]),
                    found = !0)),
                    "input" == nn && livespell.insitu.filterTextInputs(AF[i]) && ("ALL" !== oid.toUpperCase() && (AF[i].disabled || AF[i].readOnly) || (out = livespell.array.safepush(out, AF[i]),
                    found = !0)),
                    "div" != nn && "iframe" != nn || livespell.insitu.filerEditors(AF[i]) && ("ALL" !== oid.toUpperCase() && (AF[i].disabled || AF[i].readOnly) || (out = livespell.array.safepush(out, AF[i]),
                    found = !0))
                }
            if ("TEXTAREAS" === oid.toUpperCase())
                ...
                i = 0; i < AF.length; i++)
                    livespell.insitu.filterTextAreas(AF[i]) && (found = !0,
                    out = livespell.array.safepush(out, AF[i]));
            else if ("TEXTINPUTS" === oid.toUpperCase())
                for (found = !0,
                AF = document.getElementsByTagName("input"),
                i = 0; i < AF.length; i++)
                    livespell.insitu.filterTextInputs(AF[i]) && (found = !0,
                    out = livespell.array.safepush(out, AF[i]));
            else if ("EDITORS" === oid.toUpperCase()) {
                for (found = !0,
                AF = document.getElementsByTagName("iframe"),
                i = 0; i < AF.length; i++)
                    livespell.insitu.filerEditors(AF[i]) && (out = livespell.array.safepush(out, AF[i]),
                    found = !0);
                for (AF = document.getElementsByTagName("div"),
                i = 0; i < AF.length; i++)
                    livespell.insitu.filerEditors(AF[i]) && (out = livespell.array.safepush(out, AF[i]),
                    found = !0)
            } else if (2 === oid.toUpperCase().split(":").length && "IFRAME" == oid.toUpperCase().split(":")[0]) {
                var frameindex = Number(oid.split(":")[1]);
                frameindex < document.getElementsByTagName("iframe").length && (myFrame = document.getElementsByTagName("iframe")[frameindex],
                ...
                for (var q = document.querySelectorAll(oid), iq = 0; iq < q.length; q++)
                    found = !0,
                ...
                for (i = 0; i < AF.length; i++) {
                    ...
                    oFieldByClass.id || (oFieldByClass.id = "livespell_CLASSSECTOR_id_" + cname + "_" + i),
            ...
            if (!found)
                if (document.getElementById(oid)) {
                    ...
                    oid.id ? out = livespell.array.safepush(out, oid) : oid.name ? (oid.id = "livespell____" + oid.name,
                    out = livespell.array.safepush(out, "livespell____" + oid.name)) : 1 == document.getElementsByName(oid).length && (document.getElementsByName(oid)[0].id = "livespell____" + oid,
    ...
    this.recieveContextSpell = function() {
        for (var myFields = this.arrCleanFields(), i = 0; i < myFields.length; i++)
    ...
    this.SpellButton = function(insitu, text, Class, style) {
        ...
        return o.setAttribute("type", "button"),
        o.type = "button",
        ...
        insitu ? o.setAttribute("onclick", " livespell.spellingProviders[" + this.id() + "].CheckInSitu()") : o.setAttribute("onclick", " livespell.spellingProviders[" + this.id() + "].CheckInWindow()"),
    ...
    this.SpellLink = function(insitu, text, Class, style) {
        ...
        return o.innerHTML = text,
        insitu ? o.setAttribute("href", "javascript:livespell.spellingProviders[" + this.id() + "].CheckInSitu()") : o.setAttribute("href", "javascript:livespell.spellingProviders[" + this.id() + "].CheckInWindow()"),
        o.setAttribute("Class", Class),
        o.className = Class,
        o.setAttribute("style", style),
        holder.appendChild(o),
    ...
    this.SpellImageButton = function(insitu, image, rollover, text, Class, style) {
        ...
        text || (text = "Spell Check"),
        Class || (Class = ""),
        image || (image = "themes/buttons/spellicon.gif",
        rollover = "themes/buttons/spelliconover.gif"),
        style || (style = "");
        var holder = document.createElement("span")
          , o = document.createElement("img");
        return o.setAttribute("alt", text),
        o.alt = text,
        ...
        rollover && o.setAttribute("onmouseout", "this.src='" + livespell.installPath + image + "'"),
        insitu ? o.setAttribute("onclick", "livespell.spellingProviders[" + this.id() + "].CheckInSitu()") : o.setAttribute("onclick", "livespell.spellingProviders[" + this.id() + "].CheckInWindow()"),
        o.setAttribute("Class", Class),
        o.className = Class,
...
function JavaScriptSpellCheckObj($setup) {
    ...
    this.SpellCheckAsYouType = function(Fields) {
        ...
        o.ActivateAsYouTypeOnLoad()) : o.ActivateAsYouType(),
        o
    }
    ,
    ...
    this.BinSpellCheckFields = function(Fields) {
        var o = this.createInstance(Fields, arguments);
        return result = o.BinSpellCheckFields(),
        result
    }
    ,
    this.ListDictionaries = function() {
        var o = this.initAJAX();
        return result = o.ListDictionaries(),
        result
    }
    ,
    ...
    this.LiveFormValidation = function(Fields, MessageHolder) {
        ...
        if (document.readyState && "complete" !== document.readyState.toLowerCase())
        ...
        for (var i = 0; i < ff.length; i++) {
            ...
            oField && (oField.setAttribute("isValidated", !0),
            oField.MessageHolder = MessageHolder,
            "string" == typeof MessageHolder && (MessageHolder = document.getElementById(MessageHolder)),
            MessageHolder && MessageHolder.id && (MessageHolder.style.display = "none"),
    ...
    this.LiveValidateMech4Proxy = function(oField, result) {
        ...
        return "string" == typeof MessageHolder && (MessageHolder = document.getElementById(MessageHolder)),
        !(!MessageHolder || !MessageHolder.id) && (MessageHolder.style.display = result ? "none" : "inherit",
    ...
    this.findf = function(Fields, PassedArgs) {
        ...
        var inputs = []
          , outputs = [];
        PassedArgs ? inputs = PassedArgs : (inputs = [],
        inputs[0] = Fields);
        for (var i = 0; i < inputs.length; i++) {
            var ff, f = inputs[i];
            f.push ? ff = f : f.split ? ff = f.split(",") : (ff = [],
            ff[0] = f);
            for (var j = 0; j < ff.length; j++) {
                ...
                if ("string" == typeof Fields) {
                    ...
                    "ALL" !== td && "EDITORS" !== td && "TEXTAREAS" !== td && "TEXTINPUTS" !== td || (outputs.push(td),
                    binWasMacro = !0),
                    "." === td.charAt(0) && (outputs.push(tdcase),
                    binWasMacro = !0)
                }
                if (!binWasMacro)
                    if (finderItem.nodeName)
                        oObject = finderItem;
                    else if (finderItem = finderItem.toString().replace(/^\s\s*/, ""),
                ...
                oObject && (oObject.id || (oObject.id = "jsspellcheck__element__" + this.findcounter,
                this.findcounter++),
...
function setup___livespell() {
    ...
    "/" == path && (path = ""),
    "/" != path && "" != path && path && !livespell.installPath ? (livespell.installPath = path,
    livespell.lang.load("en")) : livespell.lang.load("en");
    ...
    livespell.test.IE5() || livespell.test.IE6() || livespell.test.IE7() || livespell.test.IE8() ? setInterval(livespell.heartbeat, 1200) : setInterval(livespell.heartbeat, 500),
    window.getComputedStyle || document.write('<script type="text/vbscript">\nsub document_oncontextmenu() \n on error resume next \ndim Oelement   \n  Set   Oelement = window.event.srcElement \n  IF(   (Oelement.className="livespell_redwiggle" OR Oelement.className="livespell_greenwiggle")) THEN \n   window.event.returnValue = false   \n     window.event.cancelBubble = true\n END IF \n end sub\n </script>'),
    livespell.test.MAC() && (livespell.events.add(document, "keydown", livespell.context.mackeydown, !1),
...
if ("undefined" == typeof livespell) {
    var livespell = {
        ...
        setrubberRingServerModel: function() {
            ...
            return livespell.ajax.debug("<h1>Spell Check Server Mode Detected & Changed to " + mode.toUpperCase() + "</h1>", !1),
        ...
        testSyncRequest: function(posturl) {
            ...
            return 200 == xhr.status ? ("undefined" != typeof window.LIVESPELL_DEBUG_MODE && livespell.ajax.debug("SERVER ERROR:" + xhr.responseText, !1),
            "no command" == xhr.responseText.toLowerCase().replace(/^\s\s*/, "").replace(/\s\s*$/, "")) : ("undefined" != typeof window.LIVESPELL_DEBUG_MODE && livespell.ajax.debug("SERVER CANNOT WORK:" + xhr.responseText, !1),
            !1)
        },
        addJs: function(js) {
            ...
            for (var id, DesiredActive = new Array, p = 0; p < livespell.spellingProviders.length; p++) {
                ...
                if (provider.AsYouTypeIsActive)
                    for (var flist = provider.arrCleanFields(), f = 0; f < flist.length; f++)
                        ...
                        "textarea" != document.getElementById(id).nodeName.toLowerCase() || document.getElementById(id + livespell.insitu._FIELDSUFFIX)) {
                            var oProx = document.getElementById(id + livespell.insitu._FIELDSUFFIX);
                            if (oProx) {
                                var obase = document.getElementById(id);
                                obase.readOnly == oProx.readOnly && obase.disabled == oProx.disabled || (livespell.insitu.destroyProxy(id),
        ...
        getIframeDocument: function(myFrame) {
            ...
            try {
                ...
                if ('javascript:"<html></html>"' == myFrame.src)
                    return oDoc;
                if (myFrame.src.toLowerCase().indexOf("javascript:") > -1)
                    return oDoc;
                oHTML = oDoc.getElementsByTagName("html")[0];
                try {
                    if (isEditable = "true" === oHTML.contentEditable || oHTML.contentEditable === !0 || "on" == oHTML.designMode || "On" == oHTML.designMode || "ON" == oHTML.designMode || oHTML.designMode === !0 || "true" === oHTML.designMode || oHTML.contentEditable === !0 || "on" == oHTML.designMode || oHTML.designMode === !0 || "true" === oHTML.designMode)
                        return oDoc
                } catch (e) {}
                if (oBody = oDoc.body,
                isEditable = "true" === oBody.contentEditable || oBody.contentEditable === !0 || "on" == oBody.designMode || "On" == oBody.designMode || "ON" == oBody.designMode || oBody.designMode === !0 || "true" === oBody.designMode || oDoc.contentEditable === !0 || "on" == oDoc.designMode || oDoc.designMode === !0 || "true" === oDoc.designMode)
                    return oDoc;
                var ofsrc = myFrame.src;
                if (myFrame = myFrame.contentWindow ? myFrame.contentWindow : myFrame,
                myFrame.frames.length) {
                    for (var oSubDoc, i = 0; i < myFrame.frames.length; i++) {
                        var mySubFrame = myFrame.frames[i];
                        if (oSubDoc = mySubFrame.contentDocument ? mySubFrame.contentDocument : mySubFrame.document,
                        mySubFrame.contentDocument ? oSubDoc = mySubFrame.contentDocument : mySubFrame.contentWindow && (oSubDoc = mySubFrame.contentWindow.document),
                        oBody = oSubDoc.body,
                        ofsrc.indexOf("fckeditor.html") > 0)
                            return oBody;
                        if (isEditable = "false" === oBody.spellcheck || oBody.spellcheck === !1 || "true" === oBody.contentEditable || oBody.contentEditable === !0 || "on" == oBody.designMode || "On" == oBody.designMode || "ON" == oBody.designMode || oBody.designMode === !0 || "true" === oBody.designMode || oDoc.contentEditable === !0 || "on" == oDoc.designMode || oDoc.designMode === !0 || "true" === oDoc.designMode)
                            return oSubDoc
                    }
                    if (isEditable = "false" === oBody.spellcheck || oBody.spellcheck === !1)
                        return oDoc
                }
            } catch (e) {}
            return null
        ...
        lang: {
            ...
            load: function(lang) {
                var idname = "__livespell__translations__" + lang
                  , fileref = E$(idname);
                fileref || (fileref = document.createElement("script"),
        ...
        ajax: {
            ...
            resend: function() {
                1 == livespell.isactiveX && livespell.test.IE() && (livespell.degradeToIframe = !0),
            ...
            send: function(cmd, args, lan, note, sender) {
                ...
                if ("asp.net" === serverModel)
                    posturl += "default.ashx";
                else if ("aspx" === serverModel)
                    posturl += "Default.aspx";
                else if ("asp" === serverModel)
                    posturl += "default.asp";
                else if ("php" === serverModel)
                    posturl += "index.php";
                else if ("auto" === serverModel || "" === serverModel)
                    posturl += "";
                else {
                    if ("" === serverModel)
                ...
                var settingsfile = livespell.spellingProviders[sender].SettingsFile
                  , hasajax = !1
                  , xhr = !1;
                oSender.BypassAuthentication && livespell.test.IE() || (xhr = livespell.ajaxClient(!1)),
                xhr && (hasajax = !0);
                try {
                ...
                hasajax) {
                    ...
                    params += "&script=true",
                    livespell.ajax.debug("URL:" + posturl + "  <br/>  GET (IFRAME):" + params, !0);
                    var theframe = E$(livespell.constants._IFRAME);
                    try {
                        var framehref = theframe.contentWindow.location.href;
                        framehref.indexOf("about:blank") < 0 && (posturl = framehref.split("?")[0]),
            ...
            send_sync: function(cmd, args, lan, note, sender) {
                ...
                if (!xhr)
                    return livespell.ajax.debug("SYNC REQUEST ERROR: No HTMLHTTP object or ActiveX", !1),
                    null;
                var params = "";
                try {
                    args = encodeURIComponent(args)
                ...
                xhr.open("GET", posturl + "?" + params, !1),
                xhr.send(),
                livespell.ajax.debug("REMOTE-CALL:" + posturl + "  <br/>  GET:" + params, !0),
                200 == xhr.status)
                    return livespell.ajax.pickup(xhr.responseText, !0);
                if (livespell.setrubberRingServerModel(),
                "undefined" != typeof window.LIVESPELL_DEBUG_MODE) {
                    try {
                        xhr.responseText.length ? livespell.ajax.debug("SERVER ERROR:" + xhr.responseText, !1) : livespell.ajax.debug("SERVER ERROR - Please view this page in a browser other than IE for full details", !1)
                    } catch (e) {
            ...
            debug: function(msg, mtcolor) {
                if ("undefined" != typeof window.LIVESPELL_DEBUG_MODE) {
                    ...
                    o.innerHTML = "<div style='background-color:" + (mtcolor === !0 ? "#ffe" : "#aab") + "'>" + o.innerHTML + msg + "</div>"
                }
            },
            needsInstantSuggestion: !1,
            pickup: function(strHTML, binInstant) {
                if (this.debug(strHTML, !1),
                ...
                var i, j, k, t, r, newSuggestions, sug_each_word, Suggestions, arrResult = strHTML.split(livespell.str.chr(5)), command = arrResult[0], vSender = Number(arrResult[1]), oSender = livespell.spellingProviders[vSender], vLang = oSender.Language;
                if (livespell.cache.suggestions[vLang] || (livespell.cache.suggestions[vLang] = []),
                livespell.cache.spell[vLang] || (livespell.cache.spell[vLang] = []),
                livespell.cache.reason[vLang] || (livespell.cache.reason[vLang] = []),
                "CTXSPELL" === command) {
                    for (t = arrResult[2].split(""),
                    r = arrResult[3].split(""),
                    i = 0; i < t.length; i++)
                        livespell.cache.reason[vLang][livespell.cache.wordlist[vSender][i]] = r[i].toString(),
                        livespell.cache.spell[vLang][livespell.cache.wordlist[vSender][i]] = "T" === t[i];
                    oSender.recieveContextSpell()
                } else if ("CTXSUGGEST" === command) {
                    for (newSuggestions = arrResult[2].split(livespell.str.chr(2)),
                    livespell.cache.suggestions[vLang][livespell.cache.suggestionrequest.word] = newSuggestions,
                    j = 0; j < newSuggestions.length; j++)
                        for (livespell.cache.spell[vLang][newSuggestions[j]] = !0,
                        sug_each_word = newSuggestions[j].replace(/\-/g, " ").split(" "),
                        k = 0; k < sug_each_word.length; k++)
                            livespell.cache.spell[vLang][sug_each_word[k]] = !0;
                    arrResult[3] && arrResult[3].length && (livespell.cache.langs = arrResult[3].split(livespell.str.chr(2))),
                    ...
                    for (Suggestions = arrResult[2].split(livespell.str.chr(1)),
                    i = 0; i < livespell.cache.suglist.length; i++)
                        for (newSuggestions = Suggestions[i].split(livespell.str.chr(2)),
                    ...
                    for (Suggestions = arrResult[4].split(livespell.str.chr(1)),
                    t = arrResult[2].split(""),
                    r = arrResult[3].split(livespell.str.chr(1)),
                    i = 0; i < t.length; i++)
                        if (livespell.cache.reason[vLang][livespell.cache.wordlist[vSender][i]] = r[i],
                        livespell.cache.spell[vLang][livespell.cache.wordlist[vSender][i]] = "T" === t[i],
                        !livespell.cache.spell[vLang][livespell.cache.wordlist[vSender][i]] && i < Suggestions.length)
                            for (newSuggestions = Suggestions[i].split(livespell.str.chr(2)),
                            ...
                            j = 0; j < newSuggestions.length; j++)
                                ...
                                k = 0; k < sug_each_word.length; k++)
                                    livespell.cache.spell[vLang][sug_each_word[k]] = !0;
                    arrResult[5] && arrResult[5].length && (livespell.cache.langs = arrResult[5].split(livespell.str.chr(2))),
                    ...
                    if ("SAVEWORD" === command) {
                        ...
                        for (i = 0; i < outInput.length; i++)
                            ...
                            livespell.cache.spell[vLang][outInput[i]] = outSpellingOk[i] = "T" === t[i];
                        if (binInstant)
                            return isValid;
                        ...
                        var outInput = arrResult[5].split(livespell.str.chr(1))
                          ...
                          , outReason = [];
                        for (i = 0; i < outInput.length; i++)
                            if (livespell.cache.reason[vLang][outInput[i]] = outReason[i] = r[i],
                            livespell.cache.spell[vLang][outInput[i]] = outSpellingOk[i] = "T" === t[i],
                        ...
                        if (outInput.length > 1 || "APISPELLARRAY" === command) {
                            if (binInstant)
                                return livespell.ajax.needsInstantSuggestion ? outSuggestions : outSpellingOk;
                            oSender.onSpellCheck(outInput, outSpellingOk, outReason, outSuggestions)
                        } else {
                            if (binInstant)
                                return livespell.ajax.needsInstantSuggestion ? outSuggestions : outSpellingOk[0];
        ...
        test: {
            ...
            isword: function(str) {
                return !!str.length && ("length" != str && (str = str.replace(/&nbsp;/g, " "),
                !(str.indexOf("<") > -1 || str.indexOf("&") > -1) && ("&nbsp;" != str && (!/[®™]+/.test(str) && (str = str.replace(/&nbsp;/g, " "),
            ...
            eos: function(str) {
                ...
                (/<br[ ]*[\/]?>/gi.test(str) || /<div[ ]*[\/]?>/gi.test(str) || /<p[ ]*[\/]?>/gi.test(str) || /[!?¿¡.][\s\S]*$$/.test(str)) && !/[.]{3}/.test(str)
            },
            nl: function(str) {
            ...
            spelling: function(word, Lang) {
                if (livespell.cache.ignore[word.toLowerCase()] && livespell.cache.ignore[word.toLowerCase()] === !0)
                    return !0;
                livespell.cache.spell[Lang] || (livespell.cache.spell[Lang] = []);
                var res = livespell.cache.spell[Lang][word];
                return res && "function" == typeof res && (res = !0),
                res
            },
            fullyCached: function(word, lang, makeSuggestions) {
                ...
                return wordSpellCheck !== !0 && (result = result && livespell.cache.reason[lang] && "undefined" != typeof livespell.cache.reason[lang][word],
                makeSuggestions && (result = result && livespell.cache.suggestions[lang] && "undefined" != typeof livespell.cache.suggestions[lang][word])),
                result
            },
        ...
        str: {
            ...
            stripSpans: function(strinput) {
                return strinput ? (strinput = strinput.replace(/(\<\/span[^>]*\>)/gi, ""),
            ...
            stripTags: function(strinput) {
                return strinput ? (strinput = livespell.str.stripComments(strinput),
            ...
            HTMLEnc: function(s) {
                return void 0 == s ? "" : (s = s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;"),
                s = s.replace(/\n/g, "<br />"),
                s = s.replace(/\r/g, ""),
                s = s.replace(/[ ][ ]/gi, " &nbsp;"),
                s = s.replace(/[ ][ ]/gi, " &nbsp;"))
            ...
            spliceSpans: function(str, pos, add) {
                ...
                for (var arrStr = str.split(""), inHTMLSpan = !1, inHTML = !0, out = "", j = 0, i = 0; i < arrStr.length; i++) {
                    var ch8r = arrStr[i];
                    try {
                        i < arrStr.length - 5 && "<" == ch8r && ("s" == arrStr[i + 1].toLowerCase() && "p" == arrStr[i + 2].toLowerCase() && "a" == arrStr[i + 3].toLowerCase() && "n" == arrStr[i + 4].toLowerCase() || "/" == arrStr[i + 1].toLowerCase() && "s" == arrStr[i + 2].toLowerCase() && "p" == arrStr[i + 3].toLowerCase() && "a" == arrStr[i + 4].toLowerCase() && "n" == arrStr[i + 5].toLowerCase()) && (inHTMLSpan = !0),
                        "<" == ch8r && (inHTML = !0)
                    } catch (e) {}
                    out += ch8r,
        ...
        userDict: {
            forget: function() {
                ...
                if (current_cookie.length)
                    for (var arrPersonalWords = current_cookie.split(livespell.str.chr(1)), i = 0; i < arrPersonalWords.length; i++)
                        livespell.cache.ignore[arrPersonalWords[i].toLowerCase()] && delete livespell.cache.ignore[arrPersonalWords[i].toLowerCase()]
            },
            load: function() {
                var current_cookie = livespell.cookie.get("SPELL_DICT_USER");
                if (current_cookie.length)
            ...
            add: function(word) {
                ...
                var current_cookie = livespell.cookie.get("SPELL_DICT_USER");
                current_cookie && (current_cookie = livespell.str.chr(1) + current_cookie),
        ...
        cookie: {
            ...
            get: function(check_name) {
                for (var a_all_cookies = document.cookie.split(";"), a_temp_cookie = "", cookie_name = "", cookie_value = "", b_cookie_found = !1, i = 0; i < a_all_cookies.length; i++) {
                    if (a_temp_cookie = a_all_cookies[i].split("="),
                    cookie_name = a_temp_cookie[0].replace(/^\s+|\s+$$/g, ""),
                    cookie_name === check_name)
                        return b_cookie_found = !0,
                        a_temp_cookie.length > 1 && (cookie_value = unescape(a_temp_cookie[1].replace(/^\s+|\s+$$/g, ""))),
        ...
        events: {
            add: function(obj, event, callback, capture) {
                if (obj.addEventListener)
                    ...
                    obj.attachEvent && (obj.detachEvent("on" + event, callback),
    ...
    livespell.insitu = {
        ...
        extractTextWithWhitespace: function(parent) {
            for (var elem, ret = "", elems = parent.childNodes, i = 0; elems[i]; i++) {
                var elem = elems[i];
                3 === elem.nodeType || 4 === elem.nodeType ? ret += elem.nodeValue : 1 === elem.nodeType && (ret += livespell.insitu.extractTextWithWhitespace(elem),
                "P" != elem.nodeName && "DIV" != elem.nodeName || (ret += "\n"),
                "BR" == elem.nodeName && elem.nextSibling && (ret += "\n"))
            }
            return ret = ret.replace(/[\x01\x02\x7F]/g, ""),
        ...
        getProxyText: function(id) {
            ...
            return val === String.fromCharCode(10) && (val = ""),
            val.length > 0 && livespell.test.FireFox() && val.charAt(val.length - 1) == String.fromCharCode(10) && (val = val.substr(0, val.length - 1)),
            val
        },
        ...
        safeUpdateProxy: function(id, providerId) {
            return !(!livespell.insitu.proxyDOM(id) || livespell.insitu.proxyDOM(id).hasFocus) && void (livespell.insitu.hasChanged(id) && (livespell.insitu.setProxyText(id, E$(id).value),
        ...
        setProxyHTMLAndMaintainCaret: function(id, val) {
            if ("" !== val && livespell.insitu.getProxyHTML(id) !== val) {
                ...
                if (window.getSelection) {
                    ...
                    liveField.innerHTML;
                    val.toLowerCase().indexOf("<p") == -1 && (val = "<p>" + val + "</p>"),
                    ...
                    if (clickx = cursorPos.boundingLeft,
        ...
        findLiveChildrenInDOMElement: function(element) {
            ...
            for (var InnerFrames = element.getElementsByTagName("iframe"), i = 0; i < InnerFrames.length; i++) {
                ...
                livespell.getIframeDocument(ittInnerFrame) && (ittInnerFrame.id || (ittInnerFrame.id = "livespell__IframeChildof_" + uid + "_" + i),
        ...
        checkNow: function(fieldList, providerID) {
            ...
            if ("" == txt) {
                livespell.cache.spell[this.provider(providerID).Language] || (livespell.cache.spell[this.provider(providerID).Language] = [],
                    ...
                    if ("textarea" == E$(id).nodeName.toLowerCase() || "input" == E$(id).nodeName.toLowerCase() && "text" == E$(id).type) {
                        ...
                        for (var strDoc = this.getProxyText(id), tokens = livespell.str.tokenize(strDoc), lng = this.provider(providerID).Language, memsize = 0, memmax = livespell.maxURI - 257, i = 0; i < tokens.length && memsize < memmax; i++)
                            if (mem_words["_" + tokens[i]] !== !0 && livespell.test.isword(tokens[i]) === !0) {
                                ...
                                cachelookup !== !0 && cachelookup !== !1 && (mem_words["_" + tokens[i]] = !0,
        ...
        destroyProxy: function(id) {
            document.getElementById(id + this._FIELDSUFFIX) && (o = livespell.insitu.proxyDOM(id),
                ...
                if (e && !e.disabled) {
                    ...
                    if (window.getComputedStyle) {
                        ...
                        for (i = 0; i < stylesToCopy.length; i++) {
                            ...
                            "height" == attr && styleval.indexOf("px") && (styleval = Number(styleval.split("px")[0]) + 1 + "px"),
                            "width" == attr && styleval.indexOf("px") && (styleval = livespell.test.IE9() ? Number(styleval.split("px")[0]) + 6 + "px" : Number(styleval.split("px")[0]) - 1 + "px"),
                            "width" == attr && (t.attributes.width && t.attributes.width.value.indexOf("%") > -1 && (styleval = t.attributes.width.value),
                            t.style.width && t.style.width.indexOf("%") > -1 && (styleval = t.style.width)),
                            "height" == attr && (t.attributes.height && t.attributes.height.value.indexOf("%") > -1 && (styleval = t.attributes.height.value),
                            t.style.height && t.style.height.indexOf("%") > -1 && (styleval = t.style.height)),
                            "margin-left" == attr && styleval.indexOf("px") && (styleval = Number(styleval.split("px")[0]) + 1 + "px"),
                            styleval && (n.style[attr2] = styleval)
                        }
                    } else if (t.currentStyle) {
                        for (n.oneline || (n.style.overflowY = "scroll"),
                        i = 0; i < stylesToCopy.length; i++)
                            if (attr = stylesToSet[i],
                            styleval = t.currentStyle[attr])
                                try {
                                    if ("width" == attr) {
                                        ...
                                        t.attributes.width && t.attributes.width.value.indexOf("%") > -1 && (styleval = t.attributes.width.value),
                                        t.style.width && t.style.width.indexOf("%") > -1 && (styleval = t.style.width)
                                    }
                                    if ("height" == attr) {
                                        try {
                                            t.offsetHeight && (n.style.height = t.offsetHeight,
                                            n.oneline && (n.style.height = t.offsetHeight - 4))
                                        } catch (e) {}
                                        t.attributes.height && t.attributes.height.value.indexOf("%") > -1 && (styleval = t.attributes.height.value),
                                        t.style.height && t.style.height.indexOf("%") > -1 && (styleval = t.style.height)
                                    }
                                    styleval += "",
                                    styleval.toUpperCase && "AUTO" != styleval.toUpperCase() && "INHERIT" != styleval.toUpperCase() && (n.style[attr] = styleval)
                                } catch (e) {}
                        for (stylesToCopy = ["cursor", "font-size", "line-height", "font-family", "font-weight", "font-style", "color", "text-transform", "text-decoration", "line-height", "text-align", "vertical-align", "direction"],
                    ...
                    t.title && (n.title = t.title),
                    n.maxLength && (livespell.events.add(n, "keyup", function() {
                        livespell.insitu.maxCharsHandler
                    }, !1),
                    livespell.events.add(n, "keypress", livespell.insitu.maxCharsHandler, !0),
                    livespell.events.add(n, "keydown", livespell.insitu.maxCharsHandler, !1),
                    livespell.events.add(n, "paste", livespell.insitu.maxCharsHandler, !1)),
                    ...
                    livespell.events.add(n, "keypress", livespell.insitu.keypresshandler, !0),
                    livespell.events.add(n, "keydown", livespell.insitu.keyhandler, !1),
                    livespell.events.add(n, "blur", livespell.insitu.blurhandler, !1),
                    livespell.events.add(n, "focus", livespell.insitu.focushandler, !1),
                    n.unChanged = !0,
                    livespell.test.IE() && ((livespell.test.IE7() || livespell.test.IE6()) && (n.onblur = function() {
                    ...
                    n.ondrop = function(e) {
                        ...
                        return cursorPos.pasteHTML(newContent),
                    ...
                    livespell.test.FireFox() ? (n.ondrop = function(e) {
                        ...
                        selection.removeAllRanges();
                        var range = document.createRange();
                        return range.setStart(cp.offsetNode, cp.offset),
                        selection.addRange(range),
                    ...
                    n.onpaste = function() {
                        ...
                        try {
                            if (!window.getSelection)
                                return document.selection.createRange().text = livespell.str.trim(clipboardData.getData("Text")),
                                livespell.insitu.updateBase(t.id),
                                !1;
                    ...
                    if (livespell.test.IE())
                        for (; "P" == trueparent.nodeName || "H1" == trueparent.nodeName || "H2" == trueparent.nodeName || "H3" == trueparent.nodeName || "H4" == trueparent.nodeName || "H5" == trueparent.nodeName || "H6" == trueparent.nodeName; )
                            truesib = trueparent,
                    ...
                    t.readOnly)
                        ...
                        "plaintext-only" !== n.contentEditable && (n.contentEditable = "true"),
                        n.designMode = "on"
                    }
                    return n.disabled = t.disabled,
                ...
                for (var resent = !1, j = 0; j < fieldList.length; j++) {
                    ...
                    if (livespell.insitu.proxyDOM(id)) {
                        for (var token, show_error, strDoc = livespell.str.stripSpans(livespell.insitu.getProxyHTML(id)), tokens = livespell.str.tokenize(strDoc), rawtokens = livespell.str.tokenize(strDoc, "RAW"), tokens_startsentence = [], tokens_isword = [], i = 0; i < tokens.length; i++) {
                            ...
                            tokens_startsentence[i] = tokens_isword && (0 === i || livespell.test.eos(tokens[i - 1])),
                            show_error = !1;
                            ...
                            if (tokens_isword[i]) {
                                ...
                                1 != livespell.test.spelling(token, this.provider(providerID).Language) && (show_error = !0),
                                show_error && (this.provider(providerID).IgnoreAllCaps && token === token.toUpperCase() && "B" !== reason && "E" !== reason && (show_error = !1),
                                this.provider(providerID).IgnoreNumeric && livespell.test.num(token) && "B" !== reason && "E" !== reason && (show_error = !1),
                                this.provider(providerID).CaseSensitive || "C" != reason || (show_error = !1)),
                                !tokens_startsentence[i] && i > 1 && this.provider(providerID).CheckGrammar && token.toUpperCase() === tokens[i - 2].toUpperCase() && token.toUpperCase() != token.toLowerCase() && (show_error = !0,
                                reason = "R"),
                                !show_error && this.provider(providerID).CaseSensitive && this.provider(providerID).CheckGrammar && tokens_startsentence[i] && livespell.test.lcFirst(token) && (!show_error && strDoc.indexOf(".") > 0 || strDoc.indexOf("!") > 0 || strDoc.indexOf("?") > 0 || strDoc.length > 50) && (show_error = !0,
                                reason = "G")
                            }
                            if (show_error) {
                                var wiggleClass = "livespell_redwiggle";
                                "R" !== reason && "G" !== reason || (wiggleClass = "livespell_greenwiggle"),
                                livespell.test.iPhone() ? tokens[i] = "<span class='" + wiggleClass + "' onclick='this.className=\"\"' >" + token + "</span>" : tokens[i] = "<span class='" + wiggleClass + "' oncontextmenu='return false' onmousedown='return livespell.insitu.disableclick(event,\"" + providerID + "\");' onmouseup=';return livespell.insitu.typoclick(event,\"" + id + '",this,"' + reason + '","' + providerID + "\")' >" + rtoken + "</span>"
                            } else
                                tokens[i] = rtoken
                        }
        ...
        init: function() {
            livespell.insitu.initiated || (livespell.insitu.initiated = !0,
            livespell.context.renderShell(),
            livespell.events.add(window.document, "mousedown", livespell.context.hideIfNotinUse, !1),
            livespell.events.add(window.document, "keydown", livespell.context.hide, !1),
        ...
        filterTextAreas: function(Area) {
            var ok = !0;
            return "none" == Area.style.display && (ok = !1),
            "hidden" == Area.style.visibility && (ok = !1),
            !ok && Area.hasLiveSpellProxy && (ok = !0),
            Area.disabled && (ok = !1),
            ok && (Area.id || (Area.id = "livespell__textarea__" + livespell.insitu.idcount++)),
            ok
        },
        filterTextInputs: function(Area) {
            return 1 == Area.hasSpellProxy || ("text" !== Area.type.toLowerCase() || Area.disabled || "none" === Area.style.display || "hidden" === Area.style.visibility || (Area.id || (Area.id = "livespell__input__" + livespell.insitu.idcount++),
        ...
        filerEditors: function(elem) {
            if ("div" == elem.nodeName.toLowerCase()) {
                var mydiv = elem;
                if (mydiv && (1 == mydiv.getAttribute("contenteditable") || "true" === mydiv.getAttribute("contenteditable")) && (mydiv.id || (mydiv.id = "livespell_rich_div_editor_id_" + livespell.insitu.idcount++),
                mydiv.id.indexOf("__livespell_proxy") == -1))
                    return !0
            }
            if ("iframe" == elem.nodeName.toLowerCase()) {
                var myFrame = elem;
                if (oDoc = livespell.getIframeDocument(myFrame),
                oDoc)
                    return myFrame.id || (myFrame.id = "livespell_rich_editor_id_" + livespell.insitu.idcount++),
                    !0
            }
        },
        ...
        maxCharsHandler: function(event) {
            ...
            if (numChars >= maxChars) {
                ...
                if (txt + "" != "" && (hasRange = !0),
                !(hasRange || 8 == ch8r || 9 == ch8r || 46 == ch8r || (ch8r > 15 && ch8r < 32 || ch8r > 32 && ch8r < 41) && 127 != ch8r)) {
                    try {
                        event || (event = window.event)
                    } catch (e) {}
        ...
        keyhandler: function(event) {
            ...
            if (!(ch8r >= 16 && ch8r <= 31 || ch8r >= 37 && ch8r <= 40)) {
                ...
                if (me.oneline && ch8r >= 10 && ch8r <= 13)
                    ...
                    event.returnValue = !1,
                    !1;
                if (livespell.insitu.ignoreAtCursor(),
                me.unChanged = !1,
                me.hasFocus = !0,
                me.autocheck) {
                    var base_field_id = me.id.split(livespell.insitu._FIELDSUFFIX)[0];
        ...
        ignoreAtCursor: function() {
            var target;
            try {
                window.getSelection ? (target = window.getSelection().focusNode,
                "SPAN" !== target.nodeName.toUpperCase() && (target = target.parentNode)) : document.selection && (target = document.selection.createRange().parentElement())
            } catch (e) {}
            target && "SPAN" === target.nodeName.toUpperCase() && (target.className = "",
            target.onmousedown = null)
        },
        disableclick: function(event, providerID) {
            var prov = livespell.spellingProviders[providerID];
            return !(!prov || !prov.RightClickOnly || 2 == event.button) || (event.preventDefault ? event.preventDefault() : event.returnValue = !1,
            !1)
        },
        MacCommandKeyDown: !1,
        typoclick: function(event, oparent, ospan, reason, providerID) {
            var prov = livespell.spellingProviders[providerID];
            if (prov && prov.RightClickOnly && 2 != event.button)
                return !0;
            event.preventDefault ? event.preventDefault() : event.returnValue = !1,
            livespell.context.caller = ospan,
            livespell.context.callerParent = oparent;
            for (var parent, p_walker = ospan; !parent; )
                "DIV" === p_walker.nodeName.toUpperCase() && (p_walker.id + "").indexOf(livespell.insitu._FIELDSUFFIX) > -1 ? parent = p_walker : p_walker = p_walker.parentNode;
            ...
            event.pageX || event.pageY ? (posx = event.pageX,
            posy = event.pageY) : (event.clientX || event.clientY) && (posx = event.clientX + document.body.scrollLeft + document.documentElement.scrollLeft,
            posy = event.clientY + document.body.scrollTop + document.documentElement.scrollTop),
            posx += 2,
            posy += 2,
            livespell.test.IE() && (posx += 3,
            posy += 3),
    ...
    livespell.context = {
        ...
        mackeydown: function(event) {
            ...
            return event.keyCode && 224 == event.keyCode && (livespell.insitu.MacCommandKeyDown = !0),
            !1
        },
        mackeyup: function() {
            return livespell.context.MacCommandKeyDown && (livespell.insitu.MacCommandKeyDown = !1),
            !1
        },
        ...
        ignore: function() {
            ...
            var b = (livespell.context.caller,
            this.base_field_id())
        ...
        addPersonal: function() {
            var b = this.base_field_id();
            "SERVER" == this.provider().AddWordsToDictionary && livespell.ajax.send("SAVEWORD", this.currentWord(), 0, 0, this.providerID),
        ...
        showMultiLang: function() {
            ...
            for (var livelangs = livespell.insitu.provider(this.providerID).Language.split(","), i = 0; i < livespell.cache.langs.length; i++) {
                ...
                for (var j = 0; j < livelangs.length; j++)
                    livespell.cache.langs[i] === livelangs[j].replace(/^\s\s*/, "").replace(/\s\s*$/, "") && (E$("livepell__multilanguage_" + livespell.cache.langs[i]).checked = !0)
            }
        },
        ...
        showMenu: function(id, word, reason, providerID) {
            ...
            var j, action = "REPLACE", suggs = livespell.cache.suggestions[livespell.spellingProviders[providerID].Language][word];
            switch (reason) {
            case "B":
                suggs = [],
            ...
            case "R":
                ...
                break;
            case "G":
                for (suggs.length || (suggs[0] = word),
                j = 0; j < suggs.length; j++)
            ...
            suggs.length && 0 !== suggs[0].length || (suggs = [],
            suggs[0] = livespell.lang.fetch(providerID, "MENU_NOSUGGESTIONS"),
            action = "IGNORE"),
            "X" === reason && (action = "REG",
            livespell.spellingProviders[providerID].isUniPacked && (suggs = new Array("No suggestions...")));
            var dsuggs = [];
            for (j = 0; j < suggs.length; j++)
        ...
        setMultiLang: function() {
            ...
            checked.length && (this.provider().Language = checked.join(","),
        ...
        renderShell: function() {
            if (!E$(livespell.context.DOM())) {
                ...
                ,
                !this.keysTrapped && livespell.test.IE() && (livespell.events.add(document.body, "keydown", this.menukey, !1),
                this.keysTrapped = !0)
            }
        },
        menukey: function() {
            return !livespell.context.isOpen() || (event.preventDefault ? event.preventDefault() : event.returnValue = !1,
            !1)
        },
        css_list: Array(),
        renderCss: function(strTheme) {
            var idname = "__livespell__stylesheet";
            if (strTheme = strTheme ? strTheme : "classic",
            !this.css_list[strTheme]) {
                var fileref = E$(idname);
                fileref ? fileref.setAttribute("href", livespell.installPath + "themes/" + strTheme + "/context-menu.css") : (fileref = document.createElement("link"),
                fileref.setAttribute("id", idname),
                fileref.id = idname,
                ...
                document.getElementsByTagName("head")[0].appendChild(fileref),
                livespell.test.IE() && (fileref = document.createElement("link"),
                fileref.setAttribute("id", idname),
                fileref.id = idname,
                fileref.setAttribute("rel", "stylesheet"),
                fileref.setAttribute("type", "text/css"),
        ...
        render: function(suggs, action, providerID, reason) {
            ...
            for (var i, j = 0; j < suggs.length; j++)
                ...
                case "IGNORE":
                    ...
                    break;
                case "REG":
                    livespell.spellingProviders[providerID].isUniPacked ? menuHTML += '<li class="ls_sug" ><a href="#" >' + suggs[j] + "</a></li>" : "asp.net" == livespell.spellingProviders[providerID].ServerModel.toLowerCase() || "aspx" == livespell.spellingProviders[providerID].ServerModel.toLowerCase() ? (menuHTML += '<li class="ls_sug" ><a href="#"></a></li>',
                    suggs = [""]) : "asp" == livespell.spellingProviders[providerID].ServerModel.toLowerCase() ? (menuHTML += '<li class="ls_sug" ><a></a></li>',
                    suggs = [""]) : (menuHTML += '<li class="ls_sug" ><a href="#" ></a></li>',
                    suggs = [""]);
                    break;
                case "DELETE":
                    menuHTML += '<li class="ls_sug" ><a href="#" onclick="livespell.context.del(); return false" >' + suggs[j] + "</a></li>"
                }
            if (menuHTML += '<li class="ls_hr" ><hr /></li>',
            this.buttonIsHidden("menuIgnore", providerID) || "B" == reason || (menuHTML += '<li><a href="#" onclick="livespell.context.ignore(); return false">' + livespell.lang.fetch(providerID, "MENU_IGNORE") + "</a></li>"),
            this.buttonIsHidden("menuIgnoreAll", providerID) || "B" == reason || (menuHTML += '<li><a href="#"  onclick="livespell.context.ignoreAll(); return false">' + livespell.lang.fetch(providerID, "MENU_IGNOREALL") + "</a></li>"),
            this.buttonIsHidden("menuAddToDict", providerID) || "B" == reason || "E" == reason || "NONE" == livespell.spellingProviders[providerID].AddWordsToDictionary || (menuHTML += '<li><a href="#"  onclick="livespell.context.addPersonal(); return false">' + livespell.lang.fetch(providerID, "MENU_LEARN") + "</a></li>"),
            ...
            livespell.spellingProviders[providerID].ShowLangInContextMenu) {
                ...
                if (1 == livespell.MultipleDictionaries && livespell.MultipleDictionaries && (doMultipleDict = !0),
                livespell.spellingProviders[providerID].Language.indexOf(",") > 0 && (livespell.MultipleDictionaries = !0,
                doMultipleDict = !0),
                doMultipleDict) {
                    for (menuHTML += "<li><a href='javascript:livespell.context.showMultiLang()' >" + livespell.lang.fetch(providerID, "MENU_LANGUAGES") + "</a></li>",
                    ...
                    i = 0; i < livespell.cache.langs.length; i++)
                        menuHTML += "<label>",
        ...
        DimViewport: function() {
            var w = 0
              , h = 0;
            "number" == typeof window.innerWidth ? (w = window.innerWidth,
            h = window.innerHeight) : document.documentElement && (document.documentElement.clientWidth || document.documentElement.clientHeight) ? (w = document.documentElement.clientWidth,
            h = document.documentElement.clientHeight) : document.body && (document.body.clientWidth || document.body.clientHeight) && (w = document.body.clientWidth,
        ...
        DimMenu: function() {
            var o = this.DOM()
              , w = 0
              , h = 0;
            "number" == typeof o.innerWidth ? (w = o.innerWidth,
            h = o.innerHeight) : (o.clientWidth || o.clientHeight) && (w = o.clientWidth,
            h = o.clientHeight);
            var o = [];
            return o.width = w,
            o.height = h,
        ...
        suggest: function(id, word, reason, caller) {
            ...
            return livespell.cache.suggestionrequest = {},
            livespell.cache.suggestions[Lang] && livespell.cache.suggestions[Lang][word] ? livespell.context.showMenu(id, word, reason, this.providerID) : (livespell.cache.suggestionrequest.id = id,
            ...
            void livespell.ajax.send("CTXSUGGEST", word, Lang, livespell.cache.langs.length ? "" : "ADDLANGS", this.providerID))
        }
    },
    document.addEventListener && navigator && navigator.userAgent.toUpperCase().indexOf("WINDOWS") > 0 && document.addEventListener && document.addEventListener("click", livespell___FF__clickmanager, !0),
    setup___livespell()
}
"undefined" != typeof jQuery && !function($) {
    ...
    var livespell$set = function(options) {
        for (var map = livespell$defaults_map, alt = livespell$defaults_alt, i = 0; i < map.length; i++)
